...
if grep -qE '^\s*(Probe|Verbose|DeepScan|DebugListNames)\s*=\s*true' "$ROOT/ResourceRespawn/Scripts/config.lua"; then
    echo "refusing to package: a diagnostic flag is still true in config.lua"
    grep -nE '^\s*(Probe|Verbose|DeepScan|DebugListNames)\s*=\s*true' "$ROOT/ResourceRespawn/Scripts/config.lua"
    exit 1
fi
